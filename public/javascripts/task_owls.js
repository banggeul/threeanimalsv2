...
import store from '/utils/storage_owls.js'
import {instructions} from '/instructions/task_owl/instructions.js'
var sequenceSets;
import { 
  sequenceSetRandom,
...
let instTexts = [];
let nextBtns = [];
const pathToSlides = "/instructions/task_owl/";
...
let demoMsgs = {
  ...
  "Do you see where Diana is standing?",
  "If you think there's a mouse under that spot,",
...
sprite_c.src = "media_assets/owl/card_anim/mouse_yes.png";

let sprite_d = new Image();
sprite_d.onload = initDirt;
sprite_d.src = "media_assets/owl/card_anim/mouse_no.png";


function initCarrot() {
  tl_c = gsap.timeline({ onUpdate: updateCarrot, onComplete: carrotFinished, paused:true })
    .to(carrot, { frame: mouse_frames.length - 1, roundProps: "frame", repeat: 0, ease: SteppedEase.config(mouse_frames.length - 1), duration: 1, delay:0.5 }, 0);
}

function initDirt() {
  tl_d = gsap.timeline({ onUpdate: updateDirt, onComplete: dirtFinished, paused:true })
      .to(dirt, { frame: no_mouse_frames.length - 1, roundProps: "frame", repeat: 0, ease: "none", duration: 1, delay:0.5 }, 0);
}

function updateCarrot() {

  let frame = mouse_frames[carrot.frame];
...
function updateDirt() {
  ...
  let frame = no_mouse_frames[dirt.frame];
